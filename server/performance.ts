...
const actionReportSchema=z.object({task:z.literal('ACTION_V1'),status:z.literal('ready'),model:z.string().min(1),revision:z.string().min(1),policy:z.enum(['lora','numeric']).default('lora'),holdMargin:z.union([finite,z.object({flat:finite,position:finite}).strict()]),generatedAt:z.string().datetime({offset:true}),source:z.string().min(1),
