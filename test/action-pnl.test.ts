...
// Closes 100, 110, 121, 99, 90; script: open_long, add, reduce, close, hold.
function replay(closes:number[],actions:ActionName[],symbol='BTCUSD') {
  let p=flatPosition();
  const candles=closes.map((close,i)=>({time:from+i*900,open:close,high:close,low:close,close,volume:1}));
  const decisions=actions.map((action,i)=>{
    const names=optionsFor(p.side);
    p=stepPaper(p,action,closes[i],from+(i+1)*900).after;
    return {marketAsOf:iso(from+(i+1)*900),action,sideAfter:p.side,unitsAfter:p.units,price:closes[i],probabilities:Object.fromEntries(names.map(n=>[n,1/names.length]))};
  });
  return {symbol,decisions,candles};
}
const closes=[100,110,121,99,90];
...
const input=(series=[replay(closes,actions)])=>({model:'Qwen/Qwen3.5-4B',revision:'jev/action@abc1234',task:'ACTION_V1',intervalMinutes:15,holdMargin:0.4,
  from:iso(from),to:iso(from+closes.length*900),source:'BitMEX XBTUSD 1m aggregated to 15m',generatedAt:iso(from+closes.length*900+60),series});
...
test('ACTION_V1 replay fails closed on broken chains, wrong prices, gaps and bad probabilities',()=>{
  ...
  bad(s=>{s.decisions[2].price=120;},/cutoff candle close/);
...
test('ACTION_V1 report round-trips through the importer and the public performance reader',async()=>{
  ...
  try {
    ...
    writeFileSync(file,JSON.stringify(input([replay(closes,actions),replay([10,10,10,10,10],['hold','hold','open_short','hold','close'],'ETHUSD')])));
