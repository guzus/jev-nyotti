        ...
        <div className="terminal-body">
          ...
          <aside className="decision-panel" aria-label="AI 시장 판단" aria-busy={busy}>
            ...
            <div className="decision-content" aria-live="polite">
              {busy ? <div className="decision-empty thinking"><div className="thinking-orbit"><Sparkles size={27} strokeWidth={1.3} /></div><h3>{sharedLoading ? '공유된 판단을 불러오는 중' : '시장의 맥락을 읽는 중'}</h3><p>{sharedLoading ? '저장된 분석 결과를 확인하고 있어요.' : '첫 요청은 모델을 켜느라 1분 이상 걸릴 수 있어요.'}</p></div> : decision && action ? <div className={`decision-result result-${decision.action}`}>
